...
findKey = head $ filter (\(_, s) -> isInfixOf " and " s) $ map (numsToText &&& applyKey ciphertext) keyList
  where keyList = [[a,b,c] | a <- [97..122], b <- [97..122], c <- [97..122]]

applyKey :: [Int] -> [Int] -> String
applyKey text key = numsToText $ concatMap (zipWith xor key) (blockText text)
  where
    blockText [] = []
    blockText (a:b:c:t) = [a,b,c] : blockText t
    blockText (a:t) = [a:t]

numsToText :: [Int] -> String
numsToText = map (\x -> toEnum x :: Char)

textToNums :: String -> [Int]
textToNums = map fromEnum

main :: IO()
main = print $ sum $ textToNums $ snd findKey
